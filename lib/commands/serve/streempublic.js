...
function isZipValid(localzipfile) {
    try {
...
exports.prapereDevPub = async function (api, pubzipurl) {
    ...
    if (await fileExists(localzipfile)) {
        if (isZipValid(localzipfile)) {
            info('MainFrame Xpublic zip already exists and is valid. Skipping download.');
        } else {
            warn('Existing MainFrame Xpublic zip is malformed. Re-downloading...');
            await fs.promises.unlink(localzipfile);
