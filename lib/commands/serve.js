...
module.exports = (api, options) => {
    ...
    api.registerCommand('serve', {
        ...
        usage: 'sf-cli-service serve [options] [entry]',
        options: {
        ...
        const prapereDevPub = require('./serve/streempublic').prapereDevPub;
        ...
        const pubDevFile = 'mainframe_xpublic.zip';

        if (!process.env.VUE_APP_MAINFRAME_URL) {
            throw new Error('VUE_APP_MAINFRAME_URL enviroment not found. Please sepecify an url of Main Frame URL');
        }
        ...
        if (entry) {
            ...
            webpackConfig.entry = {index: [require('./build/resolvePages').resolvePagesEntry(api)]}
        }


       const pubdevfolder=  await prapereDevPub(api,  pubDevFile);

        webpackConfig.plugins.forEach(x => {
            if(x.__pluginName ==='html'){
                x.options.template = pubdevfolder + '/index.html'
            }

            if(x.__pluginName ==='copy'){
                x.patterns[0].force = true;
                x.patterns.push({
                    from: pubdevfolder,
                    to: api.resolve('dist'),
                    toType: "dir"
        ...
        compiler.hooks.failed.tap('sf-cli-service serve', msg => {
            error(msg)
            process.exit(1)
        })
        ...
        return new Promise((resolve, reject) => {
            ...
            compiler.hooks.done.tap('sf-cli-service serve', stats => {
